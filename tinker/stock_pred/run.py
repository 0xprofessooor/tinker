...
class StateModel(nnx.Module):
    def __init__(
        ...
        self.gru_cell = nnx.GRUCell(
            in_features=(obs_dim * 2) + action_dim,
        ...
        self.vol_head = nnx.Sequential(
            ...
            nnx.silu,
            nnx.Linear(
                128, obs_dim, kernel_init=output_init, bias_init=zero_bias, rngs=rngs
            ),
        )

    def get_h(self, h: jax.Array, obs: jax.Array, action: jax.Array) -> jax.Array:
        gru_input = jnp.concatenate([obs, jnp.square(obs), action], axis=-1)
    ...
    def __call__(
        ...
        base_quantiles = out.squeeze(-1)

        pred_var = jax.nn.softplus(self.vol_head(h_next))
        pred_std = jnp.sqrt(pred_var + 1e-8)

        next_obs_quantiles = pred_std * base_quantiles

        return h_next, next_obs_quantiles, pred_var
        ...
        def _update_step(runner_state: tuple, update_idx: int):
            ...
            def _update_epoch(runner_state: tuple, _):
                ...
                def loss_fn(model: StateModel) -> jax.Array:
                    ...
                    _, pred_obs, pred_var = model(batch_h, batch_obs, batch_action, tau)

                    delta = target_obs - pred_obs
                    pinball_loss = jnp.mean(
                        jnp.sum(
                            delta * (tau - (delta < 0).astype(jnp.float32)), axis=-1
                        )
                    )

                    # NLL assuming zero mean
                    eps = 1e-6
                    safe_pred_var = pred_var + eps
                    vol_loss = jnp.mean(
                        jnp.log(safe_pred_var) + jnp.square(target_obs) / safe_pred_var
                    )

                    total_loss = pinball_loss + (config.vol_loss_coeff * vol_loss)
                    return total_loss
...
if __name__ == "__main__":
    ...
    dynamic_config = DynamicConfig(
        ...
        vol_loss_coeff=jnp.ones(NUM_SEEDS) * 1.0,
    )
