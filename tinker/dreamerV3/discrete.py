"""DreamerV3 with Discrete Action Space.

A single-file, end-to-end JIT-compilable JAX implementation of DreamerV3
for discrete-action gymnax environments. Follows the tinker project
...
- Categorical action distribution with Gumbel-Softmax and unimix
"""
...
def twohot_encode(target, bins):
    """Bins are already in symexp space; targets compared directly."""
...
def twohot_decode(logits, bins):
    """Bins are already in symexp space; weighted average gives the value directly."""
...
# ---------------------------------------------------------------------------
# Full DreamerV3 model (discrete actions)
# ---------------------------------------------------------------------------
...
class DreamerV3(nnx.Module):
    def __init__(
        ...
        self.unimix = 0.01
        self.actor_unimix = 0.01

        self.encoder = MLPStack(obs_dim, hidden, 3, rngs, symlog_inputs=True)
        self.deter_net = DeterNet(deter, hidden, self.flat_stoch, num_actions, rngs)
    ...
    def obs_step(self, stoch, deter, action_oh, embed, reset, rng):
        stoch = jnp.where(reset[..., None, None], 0.0, stoch)
        deter = jnp.where(reset[..., None], 0.0, deter)
        action_oh = jnp.where(reset[..., None], 0.0, action_oh)
        stoch_flat = stoch.reshape(*stoch.shape[:-2], self.flat_stoch)
        deter = self.deter_net(stoch_flat, deter, action_oh)
        logits = self.obs_net(deter, embed).reshape(
            *deter.shape[:-1], self.stoch, self.discrete
        )
        logits = self._unimix(logits)
        new_stoch = gumbel_softmax(rng, logits, hard=True)
        return new_stoch, deter, logits

    def img_step(self, stoch, deter, action_oh, rng):
        stoch_flat = stoch.reshape(*stoch.shape[:-2], self.flat_stoch)
        deter = self.deter_net(stoch_flat, deter, action_oh)
        logits = self.img_net(deter).reshape(
            *deter.shape[:-1], self.stoch, self.discrete
        )
        logits = self._unimix(logits)
        new_stoch = gumbel_softmax(rng, logits, hard=True)
    ...
    def actor(self, feat):
        logits = self.actor_out(self.actor_mlp(feat))
        probs = jax.nn.softmax(logits, axis=-1)
        uniform = self.actor_unimix / self.num_actions
        probs = probs * (1.0 - self.actor_unimix) + uniform
        return jnp.log(probs)
...
def make_train(
    ...
    :param num_epochs: Gradient updates per collection phase.
    ...
    def train(config: DynamicConfig) -> Tuple[dict, dict]:
        ...
        prev_action = jnp.zeros((num_envs, num_actions))

        buf_size = min(num_steps, 100_000)
        buf = {
            "obs": jnp.zeros((buf_size, obs_input_dim)),
            "action": jnp.zeros((buf_size, num_actions)),
        ...
        def _update_step(carry, _):
            ...
            def _env_step(carry, _):
                ...
                action_oh = jax.nn.one_hot(action_int, num_actions)

                step_rngs = jax.random.split(step_rng, num_envs)
                next_obs, env_state, reward, done, info = jax.vmap(
                    env.step, in_axes=(0, 0, 0, None)
                )(step_rngs, env_state, action_int, config.env_params)
            ...
            def _train_epoch(train_carry, _):
                ...
                def _loss_fn(params, slow_params, ret_ema, rng):
                    ...
                    imag_reward = twohot_decode(
                        model.reward(imag_feat_flat).reshape(BT, imag_steps, num_bins),
                        bins,
                    )
                    ...
                    imag_value = twohot_decode(
                        model.critic(imag_feat_flat).reshape(BT, imag_steps, num_bins),
                        bins,
                    )
                    imag_slow_value = twohot_decode(
                        slow_model.critic(imag_feat_flat).reshape(
                    ...
                    logpi = (imag_actions_sg * log_probs_all).sum(
                    ...
                    repval_slow_value = twohot_decode(
                        slow_model.critic(repval_feat_flat).reshape(B, T, num_bins),
                        bins,
                    )
                ...
                (_, (epoch_metrics, ret_ema)), grads = jax.value_and_grad(
